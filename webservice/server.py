...
TYPING_DATA_PATH = './database/biometria.csv'
...
def biometria():
	if request.method == 'POST':
		...
		try:
			...
			return jsonify({'biometric_cod': 'Unable to register biometric data'})
...
def treinar():
	if request.method == 'POST':
		...
		if user_id == None: #If a user not registered in the training is entered, the data can still be used.
			user_id = 999 
		data.append(user_id) # add the user id to the end of the list
		try:
			with open(TYPING_DATA_PATH, 'a', newline='') as file:			
				writer = csv.writer(file)
				writer.writerow(data)
...
def auth2():
	...
	with open(LOG_NAME, 'a') as arquivo: # Create the log file
