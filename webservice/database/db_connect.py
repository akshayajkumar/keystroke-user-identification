        ...
        with sql.connect("database.db") as con:
            ...
            if rows:
                for row in rows:
                    ...
                    senha = row[2]
                if senha == password:
                    print('WARNING - DATABASE - check_user_and_pass: Username and password match, authenticated user!  USER_ID:',user_id, 'USER_NAME:',usuario)
                    return 2, True, user_id
                else: 
                    print('WARNING - DATABASE - check_user_and_pass: Existing user but password does not match! USER_ID:',user_id, 'USER_NAME:',usuario)
                    return 1, False, user_id
            else:
                print("ERROR - DATABASE - check_user_and_pass: User does not exist in the bank!")
                return 3, False, 0
    except:
